...
namespace Web_QuanLyNhaHang
{
    public partial class QuanLyNhanVien : Form
    {
        ...
        {
            InitializeComponent();
            dataGridView1.SelectionChanged += dataGridView1_SelectionChanged; // Add event handler
        }
        void capNhapBang()
        {
            stt = 0;
        ...
        {
            if (dataGridView1.CurrentRow != null && dataGridView1.CurrentRow.Cells[2].Value != null)
            {
                try
                {
                    DateTime dt = DateTime.ParseExact(dataGridView1.CurrentRow.Cells[3].Value.ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture);
